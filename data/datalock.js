...
Element.prototype.wpuiIf = function(dataObject, key) {
    ...
    if (!value || isfalseString) {
        ...
        siblings.each(function(myElm) {
            if(!myElm.attr) return;
            if (myElm.attr("wpui-if")) {
                //will get filled another way
                foundTrue = true;
                return;
            }
            if (myElm.attr("wpui-elseif")) {
                var key2 = myElm.attr("wpui-elseif");
                var value2 = wpui_checkValidity(key2, dataObject);
                var isfalseString = (value2 === 'false');

                var ifElseContent = myElm.data("wpuiIfContent");
                if (!ifElseContent) {
                    ifElseContent = myElm.html();
                    myElm.data("wpuiIfContent", ifElseContent);
                }
                if (foundTrue) {
                    myElm.empty();
                    return;
                ...
                if (value2 && !isfalseString) {
                    foundTrue = true;
                    myElm.html(ifElseContent);
                    myElm.wpuiFill(dataObject);
                }
                else {
                    myElm.empty();
                }
            ...
            if (myElm.hasAttribute("wpui-else")) {
                ...
                if (!elseContent) {
                    ...
                    myElm.data("wpuiIfContent", elseContent);
                }
                if (!foundTrue) {
                    foundTrue = true;
                    myElm.html(elseContent);
                    myElm.wpuiFill(dataObject);
                }
                else {
                    myElm.empty();
        ...
        siblings.each(function(myElm) {
            ...
            if (myElm.attr("wpui-elseif") || myElm.hasAttribute("wpui-else")) {
                ...
                if (!previousContent) {
                    ...
                    myElm.data("wpuiIfContent", previousContent);
                }
                myElm.empty();
            }
        });
